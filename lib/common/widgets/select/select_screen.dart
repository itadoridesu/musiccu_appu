...
class SelectionScreen<T> extends StatelessWidget {
  ...
  final Widget? bottomNavBarWidget; // Add an optional bottom navigation bar widget
  final SelectionController<T> selectionController; // Receive controller as a parameter
  final Color? color;
  ...
  const SelectionScreen({
    ...
    this.onConfirm,
    this.bottomNavBarWidget, // Initialize the optional bottomNavBarWidget
  ...
  child: Column(
    ...
    children: [
      // "Select All" row
      Obx(() {
        final allSelected = selectionController.selectedIds.length == items.length;
        return Row(
          children: [
            Transform.scale(
              scale: 1.2,
      ...
      bottomNavigationBar: bottomNavBarWidget ?? SizedBox.shrink(), // Show the widget if provided, otherwise show nothing
    );
  }
}
